...
class DisplayActivity : AppCompatActivity() {
    ...
    private fun displayData() {
        val url= "http://10.234.211.146/PHP/viewBuku.php"
        val stringRequest = object : StringRequest(Method.GET,url, Response.Listener { response ->
            ...
            for (i in 0..jsonArray.length()-1) {
                ...
                product.gambarBuku  = "http://10.234.211.146/PHP/" + item.getString("gambarBuku")
                product.kodeKategori  = item.getString("kodeKategori")
                productList.add(product)
            }
            recyclerView.adapter = AdapterBuku(this@DisplayActivity, productList)
        },
